...
// const proxy = "http://triplepr.dev.pl"; // 'http://localhost/'
...
function style(done) {
  return (
    ...
    .pipe(
      sass({
        outputStyle: styleConfig.outputStyle
      }).on("error", function (
        err
      ) {
        notify().write(err);
    ...
    // }))'
    .pipe(
      gulpif(styleConfig.prefix, prefix({
        browsers: ["last 3 versions"]
      }))
    )
...
function images(done) {
  gulp.src("src/img/**/*").pipe(image()).pipe(gulp.dest("src/img"));
